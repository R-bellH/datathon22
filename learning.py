...
#features list
featureslist=prices.columns.tolist()[:-1]
...
print(f"linear_reg: {mean_squared_error(y_test, y_pred)**0.5}")
# random forest
from sklearn.ensemble import RandomForestRegressor
rf_reg = RandomForestRegressor(n_estimators=200, random_state=42, max_depth=75, min_samples_leaf=0.025,
                               min_samples_split=0.025, max_features='sqrt', bootstrap=True, oob_score=True,
                               n_jobs=-1)
rf_reg.fit(X_train, y_train)
y_pred = rf_reg.predict(X_test)
print(f"random_forest: {mean_squared_error(y_test, y_pred)**0.5}")
